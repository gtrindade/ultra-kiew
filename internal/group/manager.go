...
func (m *Manager) Manage(args map[string]any) (string, error) {
	...
	if callerChatID > 0 && (action == "create" || action == "remove") {
		return "", fmt.Errorf("security policy violation: groups can only be created or removed from within the actual group chat. You are currently in a private DM. Refuse the request and instruct the user to go to the group chat to perform this action.")
	}
